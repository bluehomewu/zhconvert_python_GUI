...
def resource_path(relative_path):
    ...
    """
    try:
        # PyInstaller 建立一個暫存資料夾，並將路徑儲存在 _MEIPASS 中
...
class App(customtkinter.CTk):
    ...
    def __init__(self, service_info: dict):
        ...
        self.icon_path = None  # 先初始化
        try:
            self.icon_path = resource_path("./pictures/favicon/favicon.ico")
            self.iconbitmap(self.icon_path)
        except Exception as e:
            # 如果找不到圖示檔案或格式不對，列印一個錯誤訊息但不會讓程式崩潰
        ...
        self.main_tab = MainTab(
            self.tab_view.tab("轉換"),
            self.service_info,
            self.start_conversion_thread,
            self._open_batch_window,  # 傳遞回呼
        )

        self.preferences_tab = PreferencesTab(
            self.tab_view.tab("偏好設定"), self.service_info
        )
        self.replace_tab = ReplaceTab(
            self.tab_view.tab("自訂取代"),
            save_callback=self._save_settings,
            clear_callback=self._clear_settings,
        )
        self.modules_tab = ModulesTab(
            self.tab_view.tab("詞語模組"),
            self.service_info,
            save_callback=self._save_settings,
            clear_callback=self._clear_settings,
        )
        self.summary_tab = SummaryTab(self.tab_view.tab("轉換總結"), self.service_info)

        self.batch_window = None  # 用於追蹤批次視窗實例
        self._load_settings()
    ...
    def _open_batch_window(self):
        ...
        if self.batch_window is None or not self.batch_window.winfo_exists():
            self.batch_window = BatchWindow(
                self
            )  # 傳入 self (App) 以便 BatchWindow 存取設定
        else:
    ...
    def _build_payload(self) -> dict:
        ...
        payload.update(
            {
                key: True
                for key, var in prefs.preferences_vars.items()
                if var.get() == "on"
            }
        )
        ...
        payload.update(
            {
                "jpTextConversionStrategy": prefs.jp_text_strategy_menu.get(),
                "jpStyleConversionStrategy": prefs.jp_style_strategy_menu.get(),
                "translateTabsToSpaces": int(prefs.translate_tabs_menu.get()),
                "diffContextLines": int(prefs.diff_context_menu.get()),
                "diffTemplate": prefs.diff_template_menu.get(),
            }
        )
        ...
        if tedious_text := prefs.tedious_keywords_textbox.get("1.0", "end-1c").strip():
            payload["userTediousKeywords"] = tedious_text

        # 5. 讀取自訂取代，並直接加入 payload
        repl_tab = self.replace_tab
        if protect := repl_tab.get_content(
            repl_tab.protect_replace_textbox, repl_tab.placeholders["protect"]
        ):
            payload["userProtectReplace"] = protect
        if pre := repl_tab.get_content(
            repl_tab.pre_replace_textbox, repl_tab.placeholders["pre"]
        ):
            payload["userPreReplace"] = pre
        if post := repl_tab.get_content(
            repl_tab.post_replace_textbox, repl_tab.placeholders["post"]
        ):
    ...
    def perform_conversion(self):
        ...
        result = (
            ...
            else convert_text_online(payload)
        )
            ...
            if self.settings_file.exists():
                with open(self.settings_file, "r", encoding="utf-8") as f:
                    ...
                    if protect_text := settings.get("userProtectReplace", ""):
                        ...
                        repl_tab.protect_replace_textbox.insert("1.0", protect_text)
    ...
    def _save_settings(self):
        try:
            repl_tab = self.replace_tab
            settings = {
                "userProtectReplace": repl_tab.get_content(
                    repl_tab.protect_replace_textbox, repl_tab.placeholders["protect"]
                ),
                "userPreReplace": repl_tab.get_content(
                    repl_tab.pre_replace_textbox, repl_tab.placeholders["pre"]
                ),
                "userPostReplace": repl_tab.get_content(
                    repl_tab.post_replace_textbox, repl_tab.placeholders["post"]
                ),
            }
            mod_tab = self.modules_tab
            ...
            modules_to_save = {
                ...
                != (
                    "停用"
                    if mod_tab.service_info["modules"].get(api_name, {}).get("isManual")
                    else "自動"
                )
            }
            if modules_to_save:
                settings["modules"] = modules_to_save
