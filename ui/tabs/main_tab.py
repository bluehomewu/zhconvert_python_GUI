...
class MainTab:
    def __init__(
        self, tab, service_info, start_conversion_callback, open_batch_callback
    ):  # 新增參數
        self.tab = tab
        self.service_info = service_info
        self.start_conversion_callback = start_conversion_callback
        self.open_batch_callback = open_batch_callback  # 儲存回呼
        self.current_converter = "Traditional"
        self.converter_buttons = []
        self.conversion_options = {}  # 用於儲存 名稱->API 的對應，供批次視窗使用
    ...
    def _create_input_block(self, parent, row, col):
        ...
        container.grid(row=row, column=col, padx=(10, 5), pady=(10, 5), sticky="nsew")
        container.grid_rowconfigure(1, weight=1)
        container.grid_columnconfigure(0, weight=1)
        header = self._create_header(container, "轉換前文字", ("#f3f4f6", "#4b5563"))
        ...
        self.input_textbox = customtkinter.CTkTextbox(
            container, font=("Arial", 16), wrap="word", border_width=1, corner_radius=0
        )
    ...
    def _create_output_block(self, parent, row, col):
        container = customtkinter.CTkFrame(parent, fg_color="transparent")
        container.grid(row=row, column=col, padx=(5, 10), pady=(10, 5), sticky="nsew")
        container.grid_rowconfigure(1, weight=1)
        container.grid_columnconfigure(0, weight=1)
        header = self._create_header(container, "轉換後結果", ("#f3f4f6", "#4b5563"))
    ...
    def _create_mode_block(self, parent, row, col):
        container = customtkinter.CTkFrame(parent, fg_color="transparent")
        container.grid(row=row, column=col, padx=(10, 5), pady=(5, 10), sticky="nsew")
        container.grid_rowconfigure(1, weight=1)
        container.grid_columnconfigure(0, weight=1)

        header = self._create_header(container, "轉換模式", ("#a5c5e9", "#3a7ebf"))

        # --- 新增：批次轉換按鈕 ---
        batch_btn = customtkinter.CTkButton(
            header, text="批次轉換檔案...", width=120, command=self.open_batch_callback
        )
        batch_btn.pack(side="right", padx=5)

        body = customtkinter.CTkFrame(container, border_width=1, corner_radius=0)
        body.grid(row=1, column=0, sticky="nsew")

        grouped_converters = {}
        for api_name, conv_data in self.service_info["converters"].items():
            # 儲存選項供外部使用
            self.conversion_options[conv_data["name"]] = api_name
        ...
        for cat_id, conv_list in grouped_converters.items():
            ...
            for conv in conv_list:
                btn = customtkinter.CTkButton(
                    row_frame,
                    text=conv["name"],
                    command=lambda c=conv["api"]: self._on_mode_button_click(c),
                )
                btn.pack(side="left", padx=5)
                self.converter_buttons.append({"api": conv["api"], "button": btn})
        self._set_active_converter_visuals(self.current_converter)

    def _create_subtitle_block(self, parent, row, col):
        container = customtkinter.CTkFrame(parent, fg_color="transparent")
        container.grid(row=row, column=col, padx=(5, 10), pady=(5, 10), sticky="nsew")
        container.grid_rowconfigure(1, weight=1)
        container.grid_columnconfigure(0, weight=1)
        header = self._create_header(container, "字幕樣式設定", ("#e9a5a5", "#c14242"))
        body = customtkinter.CTkFrame(container, border_width=1, corner_radius=0)
        body.grid(row=1, column=0, sticky="nsew")
        body.grid_columnconfigure(1, weight=1)
        jp_frame = customtkinter.CTkFrame(body, fg_color="transparent")
        jp_frame.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
        ...
        ignore_frame.grid(row=1, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
        ignore_frame.grid_columnconfigure(1, weight=1)
        customtkinter.CTkLabel(ignore_frame, text="不要處理的樣式").grid(
            row=0, column=0
        )
    ...
    def _create_header(self, parent, title, color):
        header = customtkinter.CTkFrame(
            parent, fg_color=color, corner_radius=0, height=40
        )
    ...
    def _on_input_keypress(self, event):
        if self.input_textbox.cget("text_color") == "gray50":
            ...
            self.input_textbox.configure(
                text_color=customtkinter.ThemeManager.theme["CTkTextbox"]["text_color"]
            )
