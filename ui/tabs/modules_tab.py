...
class ModulesTab:
    ...
    def __init__(self, tab, service_info, save_callback, clear_callback):
        ...
        for row_idx, (api_name, module_data) in enumerate(
            ...
            # 名稱
            name_label = customtkinter.CTkLabel(scrollable_frame,
                                                text=module_data.get(
                                                    'name', ''))
            name_label.grid(row=row_idx, column=0, padx=10, pady=8, sticky="w")

            # 狀態
            menu = customtkinter.CTkOptionMenu(scrollable_frame,
                                               width=100,
                                               values=["自動", "啟用", "停用"])
            ...
            desc_container = customtkinter.CTkFrame(scrollable_frame,
                                                    fg_color="transparent")
            desc_container.grid(row=row_idx,
                                column=3,
                                padx=10,
                                pady=5,
                                sticky="w")

            desc_label = customtkinter.CTkLabel(desc_container,
                                                text=module_data.get(
                                                    'desc', ''),
                                                wraplength=300,
                                                justify="left")
            ...
            if module_data.get('isManual', False):
                manual_label = customtkinter.CTkLabel(desc_container,
                                                      text="(必須手動啟用)",
                                                      text_color="#E57373")
