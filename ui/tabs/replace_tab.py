...
class ReplaceTab:
    ...
    def __init__(self, tab, save_callback, clear_callback):
        ...
        self.normal_color = customtkinter.ThemeManager.theme["CTkTextbox"][
            "text_color"]
        ...
        flow_label = customtkinter.CTkLabel(top_frame,
                                            text=flow_text,
                                            fg_color=("#e3eef9", "#212d3a"),
                                            corner_radius=6)
        flow_label.grid(row=0,
                        column=0,
                        padx=(0, 10),
                        pady=10,
                        ipady=10,
        ...
        save_button = customtkinter.CTkButton(top_frame,
                                              text="儲存設定",
                                              command=save_callback)
        save_button.grid(row=0, column=2, padx=5, pady=10)
        clear_button = customtkinter.CTkButton(top_frame,
                                               text="清除設定",
        ...
        content_frame = customtkinter.CTkFrame(self.tab,
    ...
    def _create_column(self, parent, col_index, title, header_color,
                       placeholder_text):
        column_frame = customtkinter.CTkFrame(parent, fg_color="transparent")
        column_frame.grid(row=0,
                          column=col_index,
                          padx=(0, 10 if col_index < 2 else 0),
                          sticky="nsew")
        column_frame.grid_rowconfigure(1, weight=1)
        column_frame.grid_columnconfigure(0, weight=1)

        header_frame = customtkinter.CTkFrame(column_frame,
        ...
        textbox = customtkinter.CTkTextbox(column_frame,
                                           font=("Arial", 14),
                                           corner_radius=6,
                                           border_width=1)
        ...
        textbox.bind("<KeyPress>",
                     lambda event, widget=textbox, placeholder=
                     placeholder_text: self._on_key_press(widget, placeholder))

        # 2. <FocusOut> 事件：當使用者離開時，檢查是否需要恢復提示文字
        textbox.bind("<FocusOut>",
                     lambda event, widget=textbox, placeholder=
                     placeholder_text: self._on_focus_out(widget, placeholder))
        ...
        clear_button = customtkinter.CTkButton(
            ...
            command=lambda t=textbox, p=placeholder_text: self._clear_textbox(
                t, p))
    ...
    def _clear_textbox(self, widget, placeholder):
        """清空按鈕的指令，清空後恢復提示文字"""
        widget.delete("1.0", "end")
        self._on_focus_out(widget, placeholder)  # 呼叫 on_focus_out 來恢復提示

    # --- 智慧內容獲取函式 ---

    def get_content(self, widget, placeholder):
        """獲取文字框的真實內容，如果內容是提示文字，則回傳空字串"""
