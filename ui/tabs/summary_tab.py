...
class SummaryTab:
    ...
    def __init__(self, tab, service_info):
        ...
        # --- 關鍵修改 START ---
        # 1. 重新使用 CTkTextbox
        self.diff_textbox = customtkinter.CTkTextbox(
            diff_card,
            font=("monospace", 12),
            wrap="none",
            corner_radius=0,
    ...
    def _create_header(self, parent, title):
        header = customtkinter.CTkFrame(
            parent, fg_color=("#a5c5e9", "#3a7ebf"), corner_radius=0
        )
    ...
    def update_content(self, summary_data):
        ...
        if summary_data is None:
            customtkinter.CTkLabel(
                self.params_scroll_frame, text="請先執行一次轉換以查看總結。"
            ).pack(pady=20)
        ...
        converter_display_name = (
            self.service_info.get("converters", {})
            .get(converter_api_name, {})
            .get("name", converter_api_name)
        )
        ...
        if used_modules:
            modules_frame = customtkinter.CTkFrame(
                self.params_scroll_frame, fg_color="transparent"
            )
            ...
            for i, api_name in enumerate(used_modules):
                module_info = self.service_info.get("modules", {}).get(api_name, {})
                ...
                label.grid(row=i // 3, column=i % 3, padx=2, pady=2, sticky="ew")
        else:
        ...
        if diff_content:
            ...
            self.diff_textbox.insert(
                "end", "轉換前後文字沒有任何改變，或未啟用差異比較。"
            )
    ...
    def _add_param(self, label_text, value_text):
        ...
        container = customtkinter.CTkFrame(
            self.params_scroll_frame, fg_color=("#f0f0f0", "#2b2d30")
        )
        ...
        value = customtkinter.CTkLabel(
            container, text=str(value_text), font=customtkinter.CTkFont(size=14)
        )
